...
EC2-DEPLOY="$bin"/../spark-1.5.0-bin-hadoop2.6/ec2/spark-ec2
$EC2-DEPLOY --key-pair=$KEY_PAIR --identity-file=$SSH_KEY --region=us-east-1 --zone=us-east-1a --instance-type=m3.medium --slaves 1 launch $CLUSTER_NAME
...
DEPLOY_HOSTS="$($EC2-DEPLOY get-master medium1Slave | tail -n1)"
...
sed -i 's/MASTERSERVERURL/$DEPLOY_HOSTS/g' "$bin"/ec2.conf
